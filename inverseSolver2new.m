...
function  picture = inverseSolver2new(N, M1, M2, k, theta, structure, q, lastEigenvalues)
  ...
  if structure == 1 
     ...
     %q_(abs(Y*h2) < sin(2*X*h1)/2 + 1) = abs(X(abs(Y*h2) < sin(2*X*h1)/2 + 1)).^(2/3)*h1;
  ...
  NearField = data2new(N, M1, M2, k, theta, q_);       
